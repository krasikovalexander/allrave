...
$user = "";
	...
	$this->template->title('Welcome to '.$this->config->item('customer_name'));
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run($this)) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
					...
					$data['site_name'] = $this->config->item('customer_name');
	...
	$this->load->module('layouts');
	$this->load->library('template');
	$this->template->title('Register - '.$this->config->item('customer_name'));
	$this->template
	->set_layout('login')
	...
	$this->load->module('layouts');
	$this->load->library('template');
	$this->template->title('Send Password - '.$this->config->item('customer_name'));
	$this->template
	->set_layout('login')
	...
	$this->load->module('layouts');
	$this->load->library('template');
	$this->template->title('Forgot Password - '.$this->config->item('customer_name'));
	$this->template
	->set_layout('login')
	...
	function _send_email($type, $email, &$data)
	{
		if($this->config->item('use_postmark') == 'TRUE'){
			...
			$this->postmark->from($this->config->item('customer_email'), $this->config->item('customer_name'));
			$this->postmark->to($email);

			$this->postmark->subject(sprintf($this->lang->line('auth_subject_'.$type), $this->config->item('customer_name')));
		...
		$this->email->from($this->config->item('customer_email'), $this->config->item('customer_name'));
		$this->email->reply_to($this->config->item('customer_email'), $this->config->item('customer_name'));
		$this->email->to($email);
		$this->email->subject(sprintf($this->lang->line('auth_subject_'.$type), $this->config->item('customer_name')));
